...
def scrape_novel_chapter(url, output_filename="novel_chapter.txt"):
    ...
    try:
        # Find the container div for the chapter content
        chapter_container = page_soup.find('div', class_='chapter-background')
        if chapter_container:
            # Find the H2 tag likely containing the chapter title
            # The class looks specific: mud-typography-h2 mud-typography-align-center mb-5 mud-typography-h4
            # We can simplify or use a more specific selector if needed
            h2_element = chapter_container.find('h2', class_='mud-typography-h4')
            if h2_element:
             ...
             print("  Warning: Could not find the main chapter container 'div.chapter-background'.")
    ...
    try:
        ...
        content_element = page_soup.find('div', id='chapter-text')
...
if __name__ == "__main__":
    # --- Updated Start URL ---
    start_url = "https://mystictranslations.com/n/tileas-worries/51"
    current_url = start_url

    page_counter = 0

    # --- Define Output Directory ---
    output_dir = "scraped_tileas_worries_mystic" # Changed directory name slightly
    ...
    while current_url:
        ...
        filename = os.path.join(output_dir, f"chapter_{safe_part}.txt") # Use chapter number in filename
        ...
        try:
            # Find the 'a' tag with rel="next" attribute
            next_link_tag = page_soup.find('a', rel="next", href=True)

            if next_link_tag:
                next_href = next_link_tag['href']
                # Use urljoin to handle relative URLs (like '/n/tileas-worries/52')
                potential_next_url = urljoin(current_url, next_href)

                # Basic check to prevent immediate loops back to the same page
                if potential_next_url != current_url:
                    next_url_to_visit = potential_next_url
                    print(f"Found next chapter link: {next_url_to_visit}")
                else:
                    print(f"Next link found ({potential_next_url}) but it leads to the same page. Stopping.")
            else:
                print("No 'a' tag with rel=\"next\" found. Presumed end of chapters.")

        except Exception as e:
             print(f"  Error finding next chapter link: {e}")
