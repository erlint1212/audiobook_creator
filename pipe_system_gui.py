...
try:
    ...
    print(
        "Warning: Pillow not installed. Image previews will be disabled. (pip install Pillow)"
    )
...
class PipelineGUI:
    def __init__(self, root):
        ...
        self.tts_start_chapter_var = tk.IntVar(value=1)
    ...
    def setup_dark_theme(self):
        ...
        self.style.theme_use(
            "clam"
        )  # 'clam' supports custom colors better than 'vista'
        ...
        self.style.configure(
            ".", background=bg_color, foreground=fg_color, borderwidth=0
        )
        ...
        # Accent Button Style
        self.style.configure(
            "Accent.TButton", background=accent_color, font=("Segoe UI", 9, "bold")
        )
    ...
    def create_ui(self):
        ...
        ttk.Button(
            top_frame, text="New", command=self.create_new_project, width=6
        ...
        ttk.Label(eng_sel_frame, text=" | Start Ch:").pack(side="left", padx=(15, 5))
        ttk.Spinbox(
            eng_sel_frame,
            from_=1,
            to=9999,
            textvariable=self.tts_start_chapter_var,
            width=5,
        ).pack(side="left")
        ...
        self.btn_stop = ttk.Button(
            btn_frame, text="STOP", command=self.stop_process, state="disabled"
        )
        ...
        # ==============================
        lbl = ttk.Label(
            self.tab_adapt, text="AI Scraper Generator", font=("Segoe UI", 12, "bold")
        )
        lbl.pack(pady=20)
        ...
        ttk.Button(
            self.tab_adapt,
            text="Generate Script with Gemini",
            command=self.run_adapt_tool,
            style="Accent.TButton",
        ).pack(pady=20)
        self.adapt_status = ttk.Label(
            self.tab_adapt, text="Ready", foreground="#888888"
        )
    ...
    def create_metadata_ui(self):
        ...
        ttk.Button(
            ...
            command=self.save_project_metadata,
            style="Accent.TButton",
        ).pack(side="left", fill="x", expand=True, padx=5)
        ttk.Button(
            btn_box, text="Reload from File", command=self.load_project_metadata
        ).pack(side="left", fill="x", expand=True, padx=5)
        ...
        self.lbl_cover_preview = ttk.Label(
    ...
    def get_env_for_project(self):
        ...
        env["TTS_START_CHAPTER"] = str(self.tts_start_chapter_var.get())
    ...
    def run_adapt_tool(self):
        ...
        self.adapt_status.config(
            text=f"Generating {mode}... wait...", foreground="#6366f1"
        )
