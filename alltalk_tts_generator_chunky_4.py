...
def _estimate_tokens(text, avg_chars_per_token=AVG_CHARS_PER_TOKEN):
    ...
    denominator = effective_avg_chars if effective_avg_chars > 0 else 1
    return math.ceil(len(text) / denominator)
...
def _split_into_sentences(text):
    ...
    splits = re.split(r'(?<=[.!?\"\'\)])(?=\s|\Z)', text)
    # Clean up: strip whitespace and filter empty strings
    sentences = [s.strip() for s in splits if s and s.strip()]
    # Combine sentences that might have been split incorrectly (e.g. "Mr. Smith") - basic heuristic
    # This part can be complex. A simpler approach is often sufficient for TTS chunking.
    # For now, we'll stick with the slightly improved regex split.
    # If sentence tokenization needs to be perfect, use NLTK or spaCy.
    return sentences

def _split_long_text_by_char_est(text_to_split, token_limit, avg_chars_per_token):
    """
    ...
    hard_char_limit = max(1, int(token_limit * avg_chars_per_token))

    while current_pos < text_len:
        end_pos = min(current_pos + hard_char_limit, text_len)
        chunk = text_to_split[current_pos:end_pos].strip() # Slice from original text
        if chunk:
            sub_chunks.append(chunk)

        if end_pos == current_pos: # Prevent infinite loop
            break
        current_pos = end_pos

    # Filter out potentially empty chunks again just in case
    ...
    for segment_text in raw_segments:
        ...
        if not current_accumulated_char_chunk:
            ...
            if potential_combined_len <= char_combination_limit:
                ...
                current_accumulated_char_chunk = segment_text
    if current_accumulated_char_chunk:
        intermediate_char_chunks.append(current_accumulated_char_chunk)

    # Phase 2: Process intermediate chunks for token limit using sentences
    final_chunks = []
    for char_chunk in intermediate_char_chunks:
        char_chunk = char_chunk.strip()
        if not char_chunk: continue

        total_est_tokens = _estimate_tokens(char_chunk, avg_chars_per_token_est)

        if total_est_tokens <= token_split_limit:
            final_chunks.append(char_chunk) # Chunk is fine as is
        else:
            # Chunk exceeds estimated token limit, needs splitting by sentence
            print(f"  Info: Chunk (length {len(char_chunk)} chars, est. {total_est_tokens} tokens) exceeds token limit ({token_split_limit}). Splitting by sentence.")
            sentences = _split_into_sentences(char_chunk)

            if not sentences: # If sentence splitting yielded nothing
                 print(f"    Warning: Could not split chunk into sentences (Text: '{char_chunk[:50]}...'). Using fallback char split.")
                 final_chunks.extend(_split_long_text_by_char_est(char_chunk, token_split_limit, avg_chars_per_token_est))
                 continue # Move to next char_chunk
            ...
            for sentence in sentences:
                ...
                sentence_tokens = _estimate_tokens(sentence, avg_chars_per_token_est)

                if sentence_tokens > token_split_limit:
                    # This single sentence is estimated to be too long!
                    print(f"    Warning: Sentence (length {len(sentence)} chars, est. {sentence_tokens} tokens) exceeds token limit ({token_split_limit}). Using fallback char split for this sentence.")
                    # Add any previously accumulated sentences first
                    if current_sub_chunk_sentences:
                        final_chunks.append(current_sub_chunk_sentences)
                    # Sub-split this single long sentence using the character-based method
                    final_chunks.extend(_split_long_text_by_char_est(sentence, token_split_limit, avg_chars_per_token_est))
                    current_sub_chunk_sentences = "" # Reset accumulator
                else:
                    # Check if adding this sentence would exceed the limit
                    separator_for_sentences = " " if current_sub_chunk_sentences else "" # Use space between sentences
                    potential_next_chunk_text = current_sub_chunk_sentences + separator_for_sentences + sentence
                    potential_tokens = _estimate_tokens(potential_next_chunk_text, avg_chars_per_token_est)

                    if potential_tokens <= token_split_limit:
                        # It fits, add sentence to current sub-chunk
                        current_sub_chunk_sentences = potential_next_chunk_text
                    else:
                        # Does not fit. Finalize the previous sub-chunk.
                        if current_sub_chunk_sentences: # Ensure we don't add empty strings
                            final_chunks.append(current_sub_chunk_sentences)
                        # Start new sub-chunk with the current sentence (we know it fits)
                        current_sub_chunk_sentences = sentence

            # Add the last accumulated sub-chunk if it has content
            if current_sub_chunk_sentences:
                final_chunks.append(current_sub_chunk_sentences)

    # Final filter for any potentially empty chunks introduced by logic/stripping
    final_chunks = [chunk for chunk in final_chunks if chunk]

    print(f"  Split text into {len(final_chunks)} final chunks (char_limit combine: {char_combination_limit}, token_limit split: {token_split_limit}, chars_per_token est: {avg_chars_per_token_est}).")
    return final_chunks
