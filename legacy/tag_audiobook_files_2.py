...
def clean_chapter_title(title):
    """Removes 'Chapter xxx - ' from the beginning of the title."""
...
def tag_audio_file(audio_filepath, text_filepath, track_num, total_tracks):
    """Reads title from text file, cleans it, and applies ID3 tags to the WAV audio file."""
    ...
    try:
        with open(text_filepath, 'r', encoding='utf-8') as f:
            chapter_title_raw = f.readline().strip()
        if not chapter_title_raw:
            print(f"  Warning: First line of text file is empty. Using fallback title.")
            chapter_title = f"Track {track_num:03d}"
        else:
            chapter_title = clean_chapter_title(chapter_title_raw)
        chapter_title = sanitize_tag_text(chapter_title) # Sanitize after cleaning
    ...
    try:
        ...
        # Year/Date (TDRC frame -<ctrl3348> format)
        if YEAR and re.match(r'^\d{4}$', YEAR):
            audio.tags.add(TDRC(encoding=3, text=YEAR))
        elif YEAR:
            print(f"  Warning: Year '{YEAR}' is not in<ctrl3348> format. Saving anyway.")
